...
if (isset($_POST['checktag'])) {
    ...
    for ($i = 0; $i < $tagsCount; $i++) {
        $query .= "FIND_IN_SET(?, tag1) OR FIND_IN_SET(?, tag2) OR FIND_IN_SET(?, tag3) OR FIND_IN_SET(?, tag4) OR ";
        $query .= "FIND_IN_SET(?, tag5) OR FIND_IN_SET(?, tag6) OR FIND_IN_SET(?, tag7) OR FIND_IN_SET(?, tag8) OR ";
        $query .= "FIND_IN_SET(?, tag9) OR FIND_IN_SET(?, tag10) OR ";
        $query .= "FIND_IN_SET(?, tag11) OR FIND_IN_SET(?, tag12) OR FIND_IN_SET(?, tag13) OR FIND_IN_SET(?, tag14) OR ";
        $query .= "FIND_IN_SET(?, tag15) OR FIND_IN_SET(?, tag16) OR FIND_IN_SET(?, tag17) OR FIND_IN_SET(?, tag18) OR ";
        $query .= "FIND_IN_SET(?, tag19) OR FIND_IN_SET(?, tag20) OR ";
    }
        ...
        <body>
            ...
            <?php foreach ($images as $image) { ?>
                <div class="image">
                    ...
                    <p><a href="../database/imagehandling.php?id=<?= $image['image_id']; ?>">
                        <img src="../database/imagehandling.php?id=<?= $image['image_id']; ?>" alt="" width="300px" height="200px" class="actualimage">
                    </a></p>
                    <p>Tags: <?php 
                    for ($i = 1; $i <= 20; $i++) {
